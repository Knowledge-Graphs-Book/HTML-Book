	<section id="chap-knowledge" class="chapter">
		...
		<p>A semantic schema allows for defining the meaning of high-level terms (aka <em>vocabulary</em> or <em>terminology</em>) used in the graph, which facilitates reasoning over graphs using those terms. Looking at Figure&nbsp;<? echo ref("fig:delg"); ?>, for example, we may notice some natural groupings of nodes based on the types of entities to which they refer. We may thus decide to define <em>classes</em>, such as <code>Event</code>, <code>City</code>, etc., to denote these groupings. In fact, Figure&nbsp;<? echo ref("fig:delg"); ?> already illustrates three low-level classes – <code>Open Market</code>, <code>Food Market</code>, <code>Drinks Festival</code> – grouping similar entities with an edge labelled <span class="gelab">type</span>. We may subsequently wish to capture some relations between some of these classes. In Figure&nbsp;<? echo ref("fig:classhier"); ?>, we present a class hierarchy for events where children are defined to be <em>subclasses</em> of their parents such that if we find an edge <? echo gedge("EID15","type","Food&nbsp;Festival"); ?> in our graph, we may also <em>infer</em> that <? echo gedge("EID15","type","Festival"); ?> and <? echo gedge("EID15","type","Event"); ?> hold in the graph.</p>
		...
		<p>Aside from classes, we may also wish to define the semantics of edge labels, aka <em>properties</em>. Returning to Figure&nbsp;<? echo ref("fig:delg"); ?>, we may consider that the properties <span class="gelab">city</span> and <span class="gelab">venue</span> are <em>sub-properties</em> of a more general property <span class="gelab">location</span>, such that given an edge <? echo gedge("Santa&nbsp;Lucía","city","Santiago"); ?>, for example, we may also infer that <? echo gedge("Santa&nbsp;Lucía","location","Santiago"); ?>. We may also consider, for example, that <span class="gelab">bus</span> and <span class="gelab">flight</span> are both sub-properties of a more general property <span class="gelab">connects&nbsp;to</span>. Along these lines, properties may also form a hierarchy similar to what we exemplified for classes. We may further define the <em>domain</em> of properties, indicating the class(es) of entities for nodes from which edges with that property extend; for example, we may define that the domain of <span class="gelab">connects&nbsp;to</span> is a class <code>Place</code>, such that given the previous sub-property relations, we could conclude that <? echo gedge("Arica","type","Place"); ?>. Conversely, we may define the <em>range</em> of properties, indicating the class(es) of entities for nodes to which edges with that property extend; for example, we may define that the range of <span class="gelab">city</span> is a class <code>City</code>, inferring that <? echo gedge("Arica","type","City"); ?>.</p>
		<p>A prominent standard for defining a semantic schema for (RDF) graphs is the <em>RDF Schema</em> (<em>RDFS</em>) standard&nbsp;<? echo $references->cite("RDFS"); ?>, which allows for defining subclasses, subproperties, domains, and ranges amongst the classes and properties used in an RDF graph, where such definitions can be serialised as a graph. We illustrate the semantics of these features in Table&nbsp;<? echo ref("tab-semSchema"); ?> and provide a concrete example of definitions in Figure&nbsp;<? echo ref("fig:sg"); ?> for a sample of terms used in the running example. These definitions can then be embedded into a data graph. More generally, the semantics of terms used in a graph can be defined in much more depth than seen here, as is supported by the <em>Web Ontology Language</em> (<em>OWL</em>) standard&nbsp;<? echo $references->cite("OWL2"); ?> for RDF graphs. We will return to such semantics later in Chapter&nbsp;<? echo ref("chap:deductive"); ?>.</p>
		...
		<table class="normalTable" id="tab-semSchema">
			...
			<tbody>
				...
				<tr>
					...
					<td><? echo gedge("\\(x\\)","\\(p\\)","\\(y\\)"); ?> implies <? echo gedge("\\(x\\)","type","\\(c\\)"); ?></td>
					<td><? echo gedge("venue","domain","Event"); ?></td>
				</tr>
				<tr>
					<td>Range</td>
					<td><? echo gedge("\\(p\\)","range","\\(c\\)"); ?></td>
					<td><? echo gedge("\\(x\\)","\\(p\\)","\\(y\\)"); ?> implies <? echo gedge("\\(y\\)","type","\\(c\\)"); ?></td>
		...
		<p>When declaring shapes, the data modeller may not know in advance the entire set of properties that some nodes can have (now or in the future). An <em>open shape</em> allows the node to have additional properties not specified by the shape, while a <em>closed shape</em> does not. For example, if we add the edge <? echo gedge("Santiago","founder","Pedro&nbsp;de&nbsp;Valdivia"); ?> to the graph represented in Figure&nbsp;<? echo ref("fig:delg"); ?>, then <span class="gnode">Santiago</span> only conforms to the <span class="shap">City</span> shape if the shape is defined as open (since the shape does not mention <span class="gelab">founder</span>).</p>
		<p>Practical languages for shapes often support additional boolean features, such as conjunction (<em class="sc">and</em>), disjunction (<em class="sc">or</em>), and negation (<em class="sc">not</em>) of shapes; for example, we may say that all the values of <span class="gelab">venue</span> should conform to the shape <span class="shap"><span class="sc">Venue</span> <em>and</em> (<em>not</em> <span class="sc">City</span>)</span>, making explicit that venues in the data graph should not be directly given as cities. However, shapes languages that freely combine recursion and negation may lead to semantic problems, depending on how their semantics are defined. To illustrate, consider the following case inspired by the barber paradox&nbsp;<? echo $references->cite("Labra2017"); ?>, involving a shape <span class="shap">Barber</span> whose conforming nodes <span class="gelab">shave</span> at least one node conforming to <span class="shap"><span class="sc">Person</span> <em>and</em> (<em>not</em> <span class="sc">Barber</span>)</span>. Now, given (only) <? echo gedge("Bob","shave","Bob"); ?> with <span class="gnode">Bob</span> conforming to <span class="shap">Person</span>, does <span class="gnode">Bob</span> conform to <span class="shap">Barber</span>? If <em>yes</em> – if <span class="gnode">Bob</span> conforms to <span class="shap">Barber</span> – then <span class="gnode">Bob</span> violates the constraint by not shaving at least one node conforming to <span class="shap"><span class="sc">Person</span> <em>and</em> (<em>not</em> <span class="sc">Barber</span>)</span>. If <em>no</em> – if <span class="gnode">Bob</span> does not conform to <span class="shap">Barber</span> – then <span class="gnode">Bob</span> satisfies the <span class="shap">Barber</span> constraint by shaving such a node. Semantics to avoid such paradoxical situations have been proposed based on stratification&nbsp;<? echo $references->cite("Boneva2017"); ?>, partial assignments&nbsp;<? echo $references->cite("Corman2018b"); ?>, and stable models&nbsp;<? echo $references->cite("Gelfond88"); ?>.</p>
		<p>Although validating schemata and semantic schemata serve different purposes, they can complement each other. In particular, a validating schema can take into consideration a semantic schema, such that, for example, validation is applied on the data graph including inferences. Taking the class hierarchy of Figure&nbsp;<? echo ref("fig:classhier"); ?> and the shapes graph of Figure&nbsp;<? echo ref("fig:shapeExample"); ?>, for example, we may define the target of the <span class="shap">Event</span> shape as the nodes that are of type <code>Event</code> (the class). If we first apply inferencing with respect to the class hierarchy of the semantic schema, the <span class="shap">Event</span> shape would now target <span class="gnode">EID15</span> and <span class="gnode">EID16</span>. The presence of a semantic schema may, however, require adapting the validating schema. Taking into account, for example, the aforementioned class hierarchy would require defining a relaxed cardinality on the <span class="gelab">type</span> property. Open shapes may also be preferred in such cases rather than enumerating constraints on all possible properties that may be inferred on a node.</p>
		<p>Two shapes languages have recently emerged for RDF graphs: <em>Shape Expressions</em> (<em>ShEx</em>), published as a W3C Community Group Report&nbsp;<? echo $references->cite("Prudhommeaux2014"); ?>; and <em>SHACL</em> (<em>Shapes Constraint Language</em>), published as a W3C Recommendation&nbsp;<? echo $references->cite("SHACLSpec"); ?>. These languages support the discussed features (and more) and have been adopted for validating graphs in a number of domains relating to health-care&nbsp;<? echo $references->cite("ThorntonSSGMPW19"); ?>, scientific literature&nbsp;<? echo $references->cite("HammondPT17"); ?>, spatial data&nbsp;<? echo $references->cite("Car2019"); ?>, amongst others. More details about ShEx and SHACL can be found in the book by <? echo $references->citet("Labra2017"); ?>. A recently proposed language that can be used as a common basis for both ShEx and SHACL reveals their similarities and differences&nbsp;<? echo $references->cite("Labra-Gayo2019"); ?>. A similar notion of schema has been proposed by <? echo $references->citet("Angles18"); ?> for property graphs.</p>

		<div class="formal">
			<p>We formally define shapes following the conventions of&nbsp;<? echo $references->citet("Labra-Gayo2019"); ?>.</p>
			...
			<div class="example">
				<p>The shapes schema from Figure&nbsp;<? echo ref("fig:shapeExample"); ?> can be expressed as:</p>
				<table>
					<tr>
			...
			<p>In a shapes schema, shapes may refer to other shapes, giving rise to a graph that is sometimes known as the <em>shapes graph</em>&nbsp;<? echo $references->cite("SHACLSpec"); ?>. Figure&nbsp;<? echo ref("fig:shapeExample"); ?> illustrates a shapes graph of this form.</p>
			...
			<div class="example">
				<p>Taking the graph \(G\) from Figure&nbsp;<? echo ref("fig:delg"); ?> and the shapes schema \(\Sigma\) from Figure&nbsp;<? echo ref("fig:shapeExample"); ?>, first assume an empty shapes target \(T = \{\}\). If we consider a shapes map where (e.g.) \(\sigma(\)<span class="gnode">EID15</span>, <span class="shap">Event</span>\() = 1\), \(\sigma(\)<span class="gnode">Santa Lucía</span>, <span class="shap">Venue</span>\() = 1\), \(\sigma(\)<span class="gnode">Santa Lucía</span>, <span class="shap">Place</span>\() = 1\), etc., but where \(\sigma(\)<span class="gnode">EID16</span>, <span class="shap">Event</span>\() = 0\) (as it does not have the required values for <span class="gelab">start</span> and <span class="gelab">end</span>), etc., then we see that \(G\) is valid under \(\Sigma\) and \(T\). However, if we were to define a shapes target \(T\) to ensure that the <span class="shap">Event</span> shape targets <span class="gnode">EID15</span> and <span class="gnode">EID16</span> – i.e., to define \(T\) such that \(\{ (\)<span class="gnode">EID15</span>, <span class="shap">Event</span>\(), (\)<span class="gnode">EID16</span>, <span class="shap">Event</span>\() \} \subseteq T\) – then the graph would no longer be valid under \(\Sigma\) and \(T\) since <span class="gnode">EID16</span> does not satisfy <span class="shap">Event</span>.</p>
			</div>

			<p>The semantics we present here assumes that each node in the graph either satisfies or does not satisfy each shape labelled by the schema. More complex semantics – for example, based on Kleene’s three-valued logic&nbsp;<? echo $references->cite("Corman2018b,Labra-Gayo2019"); ?> – have been proposed that support partial shapes maps, where the satisfaction of some nodes for some shapes can be left as undefined. Shapes languages in practice may support other more advanced forms of constraints, such as counting on paths&nbsp;<? echo $references->cite("SHACLSpec"); ?>. In terms of implementing validation with respect to shapes, work has been done on translating constraints into sets of graph queries, whose results are input to a SAT solver for recursive cases&nbsp;<? echo $references->cite("CormanFRS19a"); ?>.</p>
		</div>
		
		<h4 id="ssec-emergentSchema" class="subsection">Emergent schema</h4>
		<p>Both semantic and validating schemata require a domain expert to explicitly specify definitions and constraints. However, a data graph will often exhibit latent structures that can be automatically extracted as an <em>emergent schema</em>&nbsp;<? echo $references->cite("PhamPEB15"); ?> (aka <em>graph summary</em>&nbsp;<? echo $references->cite("LiuSDK18,CebiricGKKMTZ19,SpahiuPPRM16a"); ?>).</p>
		<p>A framework often used for defining emergent schema is that of <em>quotient graphs</em>, which partition groups of nodes in the data graph according to some equivalence relation while preserving some structural properties of the graph. Taking Figure&nbsp;<? echo ref("fig:delg"); ?>, we can intuitively distinguish different <em>types</em> of nodes based on their context, such as event nodes, which link to venue nodes, which in turn link to city nodes, and so forth. In order to describe the structure of the graph, we could consider six partitions of nodes: <em>event</em>, <em>name</em>, <em>venue</em>, <em>class</em>, <em>date-time</em>, <em>city</em>. In practice, these partitions may be computed based on the class or shape of the node. Merging the nodes of each partition into one node while preserving edges leads to the quotient graph shown in Figure&nbsp;<? echo ref("fig:emergentSchema"); ?>: the nodes of this quotient graph are the partitions of nodes from the data graph and the edge <? echo gedge("\\(X\\)","\\(y\\)","\\(Z\\)"); ?> is in the quotient graph if and only if there exists \(x \in X\) and \(z \in Z\) such that <? echo gedge("\\(x\\)","\\(y\\)","\\(z\\)"); ?> is in the data graph.</p>
		...
		<p>There are many ways in which quotient graphs may be defined, depending on the equivalence relation that partitions nodes. Furthermore, there are many ways in which other similar or bisimilar graphs can be defined, depending on the (bi)simulation relation that preserves the data graph’s structure&nbsp;<? echo $references->cite("CebiricGKKMTZ19"); ?>. We provide formal definitions for the notions of <em>quotient graphs</em>, <em>simulation</em> and <em>bisimulation</em> in Appendix&nbsp;<? echo ref("sec:feschema"); ?>. Such techniques aim to <em>summarise</em> the data graph into a higher-level topology. In order to reduce the memory overhead of the quotient graph, in practice, nodes may rather be labelled with the cardinality of the partition and/or a high-level label (e.g., <em>event</em>, <em>city</em>) for the partition rather than storing the labels of all nodes in the partition.</p>
		<p>Various other forms of emergent schema not based on a quotient graph framework have also been proposed; examples include emergent schemata based on relational tables&nbsp;<? echo $references->cite("PhamPEB15"); ?>, formal concept analysis&nbsp;<? echo $references->cite("GonzalezH18"); ?>, and so forth. Emergent schemata may be used to provide a human-understandable overview of the data graph, to aid with the definition of a semantic or validating schema, to optimise the indexing and querying of the graph, to guide the integration of data graphs, and so forth. We refer to the survey by <? echo $references->citet("CebiricGKKMTZ19"); ?> for further details.</p>
		...
		<p>Since identifiers can be arbitrary, it is common to add edges that provide a human-interpretable label for nodes, such as <? echo gedge("wd:Q2887","rdfs:label","“Santiago”"); ?>, indicating how people may refer to the subject node linguistically. Linguistic information of this form plays an important role in grounding knowledge such that users can more clearly identify which real-world entity a particular node in a knowledge graph actually references&nbsp;<? echo $references->cite("Lexvo"); ?>; it further permits cross-referencing entity labels with text corpora to find, for example, documents that potentially speak of a given entity&nbsp;<? echo $references->cite("IESW"); ?>. Labels can be complemented with aliases (e.g., <? echo gedge("wd:Q2887","skos:altLabel","“Santiago&nbsp;de&nbsp;Chile”"); ?>) or comments (e.g. <? echo gedge("wd:Q2887","rdfs:comment","“Santiago&nbsp;is&nbsp;the&nbsp;capital&nbsp;of&nbsp;Chile”"); ?>) to further help ground the node’s identity.</p>
		<p>Nodes such as <span class="gnode">“Santiago”</span> denote string literals, rather than an identifier. Depending on the specific graph model, such literal nodes may also be defined as a pair \((s,l)\), where \(s\) denotes the string and \(l\) a language code; in RDF, for example we may state <? echo gedge("chile:City","rdfs:label","\"City\"@en"); ?>, <? echo gedge("chile:City","rdfs:label","\"Ciudad\"@es"); ?>, etc., indicating labels for the node in different languages. In other models, the pertinent language can rather be specified, e.g., via metadata on the edge. Knowledge graphs with human-interpretable labels, aliases, comments, etc., (in various languages) are sometimes called (<em>multilingual</em>) <em>lexicalised knowledge graphs</em>&nbsp;<? echo $references->cite("BonattiDPP18"); ?>".</p>
		...
		<p class="mathblock"><span class="gnode">chile:EID42</span><? echo esource(); ?><span class="edge">chile:venue</span><? echo etipr(); ?><span class="gnode">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span><? echo etipl(); ?><span class="edge">chile:venue</span><? echo esource(); ?><span class="gnode">chile:EID43</span></p>

		<p>These edges denote that there exists a common venue for <span class="gnode">chile:EID42</span> and <span class="gnode">chile:EID42</span> without identifying it. Existential nodes are supported in RDF as blank nodes&nbsp;<? echo $references->cite("rdf11"); ?>, which are also commonly used to support modelling complex elements in graphs, such as <em>RDF lists</em>&nbsp;<? echo $references->cite("rdf11,HoganAMP14"); ?>. Figure&nbsp;<? echo ref("fig:list"); ?> exemplifies an RDF list, which uses blank nodes in a linked-list structure to encode order. Though existential nodes can be convenient, their presence can complicate operations on graphs, such as deciding if two data graphs have the same structure modulo existential nodes&nbsp;<? echo $references->cite("rdf11,Hogan17"); ;?>. Hence methods for <em>skolemising</em> existential nodes in graphs – replacing them with canonical labels – have been proposed&nbsp;<? echo $references->cite("canon,Hogan17"); ?>. Other authors rather call to minimise the use of such nodes in graph data&nbsp;<? echo $references->cite("ldbook"); ?>.</p>
		...
		<h3>Context</h3>
		<p>Many (arguably <em>all</em>) facts presented in the data graph of Figure&nbsp;<? echo ref("fig:delg"); ?> can be considered true with respect to a certain <em>context</em>. With respect to <em>temporal context</em>, <span class="gnode">Santiago</span> has only existed as a city since 1541, flights from <span class="gnode">Arica</span> to <span class="gnode">Santiago</span> began in 1956, etc. With respect to <em>geographic context</em>, the graph describes events in Chile. With respect to <em>provenance</em>, data relating to <span class="gnode">EID15</span> were taken from – and are thus said to be true with respect to – the Ñam webpage on January 4<sup>th</sup>, 2020. Other forms of context may also be used. We may further combine contexts, such as to indicate that <span class="gnode">Arica</span> is a Chilean city (<em>geographic</em>) since 1883 (<em>temporal</em>) according to the Treaty of Ancón (<em>provenance</em>).</p>
		<p>By context we herein refer to the <em>scope of truth</em>, and thus talk about the context in which some data are held to be true&nbsp;<? echo $references->cite("McCarthy93,GuhaMF04"); ?>. The graph of Figure&nbsp;<? echo ref("fig:delg"); ?> leaves much of its context implicit. However, making context explicit can allow for interpreting the data from different perspectives, such as to understand what held true in 2016, what holds true excluding webpages later found to have spurious data, etc. As seen in the previous examples, context for graph data may be considered at different levels: on individual nodes, individual edges, or sets of edges (sub-graphs). We now discuss various representations by which context can be made explicit at different levels.</p>

		<h4 id="sssec-direct-representation" class="subsection">Direct representation</h4>
		<p>The first way to represent context is to consider it as data no different from other data. For example, the dates for the event <span class="gnode">EID15</span> in Figure&nbsp;<? echo ref("fig:delg"); ?> can be seen as representing a form of temporal context, indicating the temporal scope within which edges such as <? echo gedge("EID15","venue","Santa&nbsp;Lucía"); ?> are held true. Another option is to change a relation represented as an edge, such as <? echo gedge("Santiago","flight","Arica"); ?>, into a node, such as seen in Figure&nbsp;<? echo ref("fig:fsa"); ?>, allowing to assign additional context to the relation. While in these examples context is represented in an ad hoc manner, a number of specifications have been proposed to represent context as data in a more standard way. One example is the <em>Time Ontology</em>&nbsp;<? echo $references->cite("timeOnt"); ?>, which specifies how temporal entities, intervals, time instants, etc. – and relations between them such as before, overlaps, etc. – can be described in RDF graphs in an interoperable manner. Another example is the <em>PROV Data Model</em>&nbsp;<? echo $references->cite("prov13"); ?>, which specifies how provenance can be described in RDF graphs, where entities (e.g., graphs, nodes, physical document) are derived from other entities, are generated and/or used by activities (e.g., extraction, authorship), and are attributed to agents (e.g., people, software, organisations).</p>

		<h4 id="sec-reify" class="subsection">Reification</h4>
		<p>Often we may wish to directly define the context of edges themselves; for example, we may wish to state that the edge <? echo gedge("Santiago","flight","Arica"); ?> is valid from 1956. While we could use the pattern of turning the edge into a node – as illustrated in Figure&nbsp;<? echo ref("fig:fsa"); ?> – to directly represent such context, another option is to use <em>reification</em>, which allows for making statements about statements in a generic manner (or in the case of a graph, for defining edges about edges). In Figure&nbsp;<? echo ref("fig:temporal"); ?> we present three forms of reification that can be used for modelling temporal context on the aforementioned edge within a directed edge-labelled graph&nbsp;<? echo $references->cite("HernandezHK15"); ?>. We use \(e\) to denote an arbitrary identifier representing the edge itself to which the contextual information can be associated. Unlike in a direct representation, \(e\) represents an edge, not a flight. RDF reification&nbsp;<? echo $references->cite("rdf11"); ?> (Figure&nbsp;<? echo ref("fig:reif"); ?>) defines a new node <span class="gnode">\(e\)</span> to represent the edge and connects it to the source node (via <span class="gelab">subject</span>), target node (via <span class="gelab">object</span>), and edge label (via <span class="gelab">predicate</span>) of the edge. In contrast, \(n\)-ary relations&nbsp;<? echo $references->cite("rdf11"); ?> (Figure&nbsp;<? echo ref("fig:nary"); ?>) connect the source node of the edge directly to the edge node <span class="gnode">\(e\)</span> with the label of the edge; the target node of the edge is then connected to <span class="gnode">\(e\)</span> (via <span class="gelab">value</span>). Finally, singleton properties&nbsp;<? echo $references->cite("Nguyen14"); ?> (Figure&nbsp;<? echo ref("fig:singprop"); ?>) rather use <span class="gelab">\(e\)</span> as an edge label, connecting it to a node indicating the original edge label (via <span class="gelab">singleton</span>). Other forms of reification have been proposed in the literature, including, for example, NdFluents&nbsp;<? echo $references->cite("Gimenez-GarciaZ17"); ?>. In general, a reified edge does not assert the edge it reifies; for example, we may reify an edge to state that it is no longer valid. We refer to the work of <? echo $references->citet("HernandezHK15"); ?> for further comparison of reification alternatives and their relative strengths and weaknesses.</p>
		...
		<p>As an alternative to reification, we can rather use higher-arity representations for modelling context. Taking again the edge <? echo gedge("Santiago","flight","Arica"); ?>, Figure&nbsp;<? echo ref("fig:temporal2"); ?> illustrates three higher-arity representations of temporal context. First, we can use a named graph (Figure&nbsp;<? echo ref("fig:ngraph"); ?>) to contain the edge and then define the temporal context on the graph name. Second, we can use a property graph (Figure&nbsp;<? echo ref("fig:pgc"); ?>) where the temporal context is defined as an attribute on the edge. Third, we can use <em>RDF*</em>&nbsp;<? echo $references->cite("Hartig17"); ?> (Figure&nbsp;<? echo ref("fig:rdfstar"); ?>): an extension of RDF that allows edges to be defined as nodes. Amongst these options, the most flexible is the named graph representation, where we can assign context to multiple edges at once by placing them in one named graph; for example, we can add more edges to the named graph of Figure&nbsp;<? echo ref("fig:ngraph"); ?> that are also valid from 1956. The least flexible option is RDF*, which, in the absence of an edge id, does not permit different groups of contextual values to be assigned to an edge; for example, considering the edge <? echo gedge("Chile","president","M.&nbsp;Bachelet"); ?>, if we add four contextual values to this edge to state that it was valid from 2006 until 2010 and valid from 2014 until 2018, we cannot pair the values, but may rather have to create a node to represent different presidencies (in the other models, we could have used two named graphs or edge ids).</p>
		...
		<div class="formal">
			<p>We define an annotation domain per <? echo $references->citet("zimm-etal-2012-JWS"); ?>.</p>
