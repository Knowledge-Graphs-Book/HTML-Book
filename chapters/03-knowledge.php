	<section id="chap-knowledge" class="chapter">
		...
		<p>A prominent standard for defining a semantic schema for (RDF) graphs is the <em>RDF Schema</em> (<em>RDFS</em>) standard&nbsp;<? echo $references->cite("RDFS"); ?>, which allows for defining sub-classes, sub-properties, domains, and ranges amongst the classes and properties used in an RDF graph, where such definitions can be serialised as a graph. We illustrate the semantics of these features in Table&nbsp;<? echo ref("tab-semSchema"); ?> and provide a concrete example of definitions in Figure&nbsp;<? echo ref("fig:sg"); ?> for a sample of terms used in the running example. These definitions can then be embedded into a data graph. More generally, the semantics of terms used in a graph can be defined in much more depth than seen here, as is supported by the <em>Web Ontology Language</em> (<em>OWL</em>) standard&nbsp;<? echo $references->cite("OWL2"); ?> for RDF graphs. We will return to such semantics later in Chapter&nbsp;<? echo ref("chap:deductive"); ?>.</p>
