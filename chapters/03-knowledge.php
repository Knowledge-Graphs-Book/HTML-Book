	<section id="chap-knowledge" class="chapter">
		...
		<p>Various other forms of emergent schema not directly based on a quotient graph framework have also been proposed; examples include emergent schemata based on relational tables&nbsp;<? echo $references->cite("PhamPEB15"); ?>, and baseed on formal concept analysis&nbsp;<? echo $references->cite("GonzalezH18"); ?>. Emergent schemata may be used to provide a human-understandable overview of the data graph, to aid with the definition of a semantic or validating schema, to optimise the indexing and querying of the graph, to guide the integration of data graphs, and so forth. We refer to the survey by <? echo $references->citet("CebiricGKKMTZ19"); ?> dedicated to the topic for further details.</p>
		...
		<div class="formal">
			...
			<dl class="definition" id="def-annotated-directed-edge-labelled-graph">
				...
				<dd>Letting \(D = \langle A,\oplus,\otimes,\bot,\top \rangle\) denote an idempotent, commutative semi-ring, we define an <em>annotated directed edge-labelled graph</em> (or <em>annotated directed edge-labelled graph</em>) as \(G = (V,E_A,L)\) where \(V \subset \con\) is a finite set of nodes, \(L \subset \con\) is a finite set of edge labels, and \(E \subseteq V \times L \times V \times A\) is a finite set of edges annotated with values from \(A\).</dd>
			</dl>
