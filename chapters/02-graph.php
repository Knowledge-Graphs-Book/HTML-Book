	<section id="chap-graph" class="chapter">
		...
		<p>A standardised data model based on directed edge-labelled graphs is the Resource Description Framework (RDF)&nbsp;<?php echo $references->cite("rdf11"); ?>, which has been recommended by the W3C. The RDF model defines different types of nodes, including <em>Internationalized Resource Identifiers</em> (IRIs)&nbsp;<?php echo $references->cite("rfc3987"); ?> which allow for global identification of entities on the Web; <em>literals</em>, which allow for representing strings (with or without language tags) and other datatype values (integers, dates, etc.); and <em>blank nodes</em>, which are anonymous nodes that are not assigned an identifier (for example, rather than create internal identifiers like <code>EID15</code>, <code>EID16</code>, in RDF, we have the option to use blank nodes). We will discuss these different types of nodes further in Section&nbsp;<?php echo ref("sec:identity"); ?> when we speak about issues relating to identity.</p>
		...
		<p>A heterogeneous graph&nbsp;<?php echo $references->cite("HusseinYC18,WangJSWYCY19,YangXJWHW20"); ?> (or <em>heterogeneous information network</em>&nbsp;<?php echo $references->cite("sun2011pathsim,2012Sun"); ?>) is a directed graph where each node and edge is assigned one type. Heterogeneous graphs are thus akin to directed edge-labelled graphs – with edge labels corresponding to edge types – but where the type of node forms part of the graph model itself, rather than being expressed with a relation (as seen in Figure&nbsp;<?php echo ref("fig:capital"); ?>). An edge is called <em>homogeneous</em> if it is between two nodes of the same type (e.g., <span class="gelab">borders</span> in Figure&nbsp;<?php echo ref("fig:capital"); ?>); otherwise it is called <em>heterogeneous</em> (e.g., <span class="gelab">capital</span> in Figure&nbsp;<?php echo ref("fig:capital"); ?>). Heterogeneous graphs allow for partitioning nodes according to their type, for example, for the purposes of machine learning tasks&nbsp;<?php echo $references->cite("HusseinYC18,WangJSWYCY19,YangXJWHW20"); ?>. Conversely, such graphs typically only support a many-to-one relation between nodes and types, which is not the case for directed edge-labelled graphs (see, for example, the node <span class="gnode">Santiago</span> with zero types and <span class="gnode">EID15</span> with multiple types in Figure&nbsp;<?php echo ref("fig:delg"); ?>).</p>
		...
		<p>Property graphs constitute an alternative graph model that offers additional flexibility when modelling more complex relations. Consider integrating incoming data that provide further details on which companies offer fares on which flights, allowing the board to better understand available routes between cities (for example, on national airlines). In the case of directed edge-labelled graphs, we cannot directly annotate an edge like <?php echo gedge("Santiago","flight","Arica"); ?> with the company (or companies) offering that route. But we could add a new node denoting a flight, connect it with the source, destination, companies, and mode, as shown in Figure&nbsp;<?php echo ref("fig:fsa"); ?>. Applying this modelling to all routes in Figure&nbsp;<?php echo ref("fig:delg"); ?> would, however, involve significant changes.</p>

		<p>The property graph model was thus proposed to offer additional flexibility when modelling data as a graph&nbsp;<?php echo $references->cite("Miller13,AnglesABHRV17"); ?>. A property graph allows a set of <em>property–value</em> pairs and a <em>label</em> to be associated with both nodes and edges. Figure&nbsp;<?php echo ref("fig:pg"); ?> depicts an example of a property graph with data analogous to Figure&nbsp;<?php echo ref("fig:fsa"); ?>. We use property–value pairs on edges to model the companies. The type of relation is captured by the label <code>flight</code>. We further use node labels to indicate the types of the two nodes, and property–value pairs for their latitude and longitude.</p>
		...
		<p>Property graphs are prominently used in graph databases, such as Neo4j&nbsp;<?php echo $references->cite("Miller13,AnglesABHRV17"); ?>. Property graphs can be converted to/from directed edge-labelled graphs&nbsp;<?php echo $references->cite("HernandezHK15,AnglesTT19"); ?> (per, e.g., Figure&nbsp;<?php echo ref("fig:pg"); ?>). In summary, directed edge-labelled graphs offer a more minimal model, while property graphs offer a more flexible one. Often the choice of model will be secondary to other practical factors, such as the implementations available for different models, etc.</p>
		...
		<div class="formal">
			...
			<div class="example">
				<p>Returning to Figure&nbsp;<?php echo ref("fig:pg"); ?>:</p>
				<ul>
		...
		<p>An RDF dataset is a graph dataset model standardised by the W3C&nbsp;<?php echo $references->cite("rdf11"); ?> where each graph is an RDF graph, and graph names can be blank nodes or IRIs. A prominent use-case for RDF datasets is to manage and query <em>Linked Data</em> composed of interlinked documents of RDF graphs spanning the Web. When dealing with Web data, tracking the source of data becomes of key importance&nbsp;<?php echo $references->cite("Dividino09,BonattiHPS11,zimm-etal-2012-JWS"); ?>. We will discuss Linked Data later in Section&nbsp;<?php echo ref("sec:identity"); ?> and further discuss provenance in Section&nbsp;<?php echo ref("ssec:knowledgeContext"); ?>.</p>
		...
		<div class="formal">
			...
			<div class="example">
				<p>Figure&nbsp;<?php echo ref("fig:gd"); ?> provides an example of a directed edge-labelled graph dataset \(D\) consisting of two named graphs and a default graph. The default graph does not have a name associated with it. The two graph names are <code>Events</code> and <code>Routes</code>; these are also used as nodes in the default graph.</p>
			</div>
		</div>

		<h4 id="sssec-othergraphs" class="subsection">Other graph data models</h4>
		<p>The previous models are popular examples of graph representations. Other graph data models exist with <em>complex nodes</em> that may contain individual edges&nbsp;<?php echo $references->cite("AnglesG08,Hartig14"); ?> or nested graphs&nbsp;<?php echo $references->cite("AnglesG08,n3"); ?> (sometimes called <em>hypernodes</em>&nbsp;<?php echo $references->cite("LeveneP89"); ?>). Likewise the mathematical notion of a <em>hypergraph</em> defines <em>complex edges</em> that connect sets rather than pairs of nodes. In our view, a knowledge graph can adopt any such graph data model based on nodes and edges: often data can be converted from one model to another (see Figure&nbsp;<?php echo ref("fig:fsa"); ?> vs.&nbsp;Figure&nbsp;<?php echo ref("fig:pg"); ?>). In the rest of the book, we prefer discussing directed edge-labelled graphs given their relative succinctness, but most discussion extends naturally to other models.</p>

		<h4 id="sssec-graphstore" class="subsection">Graph stores</h4>
		<p>A variety of techniques have been proposed for storing and indexing graphs, facilitating the efficient evaluation of queries (as discussed next). Directed edge-labelled graphs can be stored in relational databases either as a single relation of arity three (<em>triple table</em>), as a binary relation for each property (<em>vertical partitioning</em>), or as \(n\)-ary relations for entities of a given type (<em>property tables</em>)&nbsp;<?php echo $references->cite("WylotHCS18"); ?>. Custom (so-called <em>native</em>) storage techniques have also been developed for a variety of graph models, providing efficient access for finding nodes, edges and their adjacent elements&nbsp;<?php echo $references->cite("AnglesG08,Miller13,WylotHCS18"); ?>. A number of systems further allow for distributing graphs over multiple machines based on popular NoSQL stores or custom partitioning schemes&nbsp;<?php echo $references->cite("WylotHCS18,JankeS18"); ?>. For further details we refer to the book chapter by <?php echo $references->citet("JankeS18"); ?> and the survey by <?php echo $references->citet("WylotHCS18"); ?> dedicated to this topic.</p>
		</section>

		<section id="ssec-querying" class="section">
		<h3>Querying</h3>
		<p>A number of languages have been proposed for querying graphs&nbsp;<?php echo $references->cite("AnglesABHRV17"); ?>, including the SPARQL query language for RDF graphs&nbsp;<?php echo $references->cite("sparql11"); ?>; and Cypher&nbsp;<?php echo $references->cite("FrancisGGLLMPRS18"); ?>, Gremlin&nbsp;<?php echo $references->cite("Rodriguez15"); ?>, and G-CORE&nbsp;<?php echo $references->cite("AnglesABBFGLPPS18"); ?> for querying property graphs. We refer to <?php echo $references->citet("seifer19"); ?> for an investigation of the popularity of these languages. Underlying these query languages are some common primitives, including (basic) graph patterns, relational operators, path expressions, and more besides&nbsp;<?php echo $references->cite("AnglesABHRV17"); ?>. We now describe these core features for querying graphs in turn, starting with basic graph patterns.</p>

		<h4 id="sssec-graphpatterns" class="subsection">Basic graph patterns</h4>
		<p>At the core of every structured query language for graphs lie <em>basic graph patterns</em>&nbsp;<?php echo $references->cite("ConsensM90,AnglesABHRV17"); ?>, which follow the same model as the data graph being queried (see Section&nbsp;<?php echo ref("ssec:graphModels"); ?>), additionally allowing variables as terms.<?php echo footnote("The terms of a directed edge-labelled graph are its nodes and edge-labels. The terms of a property graph are its ids, labels, properties, and values (as used on either edges or nodes)."); ?> Terms in basic graph patterns are thus divided into constants, such as <span class="gnode">Arica</span> or <span class="gelab">venue</span>, and variables, which we prefix with question marks, such as <span class="gvar">?event</span> or <span class="gelab" style="color: black">?rel</span>. A basic graph pattern is then evaluated against the data graph by generating mappings from the variables of the graph pattern to constants in the data graph such that the image of the graph pattern under the mapping (replacing variables with the assigned constants) is contained within the data graph.</p>
		<p>Figure&nbsp;<?php echo ref("fig:gp"); ?> provides an example of a basic graph pattern looking for the venues of Food Festivals, along with the possible mappings generated by the graph pattern against the data graph of Figure&nbsp;<?php echo ref("fig:delg"); ?>. In some of the presented mappings (the last two listed), multiple variables are mapped to the same term, which may or may not be desirable depending on the application. Hence a number of semantics have been proposed for evaluating basic graph patterns&nbsp;<?php echo $references->cite("AnglesABHRV17"); ?>, amongst which the most important are: <em>homomorphism-based semantics</em>, which allows multiple variables to be mapped to the same term such that all mappings shown in Figure&nbsp;<?php echo ref("fig:gp"); ?> would be considered results; and <em>isomorphism-based semantics</em>, which requires variables on nodes and/or edges to be mapped to unique terms, thus excluding the latter three mappings of Figure&nbsp;<?php echo ref("fig:gp"); ?> from the results. Different languages may adopt different semantics for evaluating basic graph patterns; for example, SPARQL adopts a homomorphism-based semantics, while Cypher adopts an isomorphism-based semantics specifically on edges (while allowing multiple variables to map to one node).</p>
		...
		<div class="formal">
			...
			<div class="example">
				<p>Returning to the example of Figure&nbsp;<?php echo ref("fig:gp"); ?>:</p>
				<ul>
			...
			<div class="example">
				<p>Figure&nbsp;<?php echo ref("fig:gp"); ?> enumerates all of the mappings given by the evaluation of the depicted basic graph pattern over the data graph of Figure&nbsp;<?php echo ref("fig:delg"); ?>. Each non-header row indicates a mapping \(\mu\).</p>
			</div>
		...
		<p>Graph query languages such as SPARQL&nbsp;<?php echo $references->cite("sparql11"); ?> and Cypher&nbsp;<?php echo $references->cite("FrancisGGLLMPRS18"); ?> allow the full use of relational operators over the results of graph patterns, giving rise to <em>complex graph patterns</em>&nbsp;<?php echo $references->cite("AnglesABHRV17"); ?>. Figure&nbsp;<?php echo ref("fig:cq"); ?> presents an example of a complex graph pattern with projected variables in bold, choosing particular variables to appear in the final results. In Figure&nbsp;<?php echo ref("fig:cgp"); ?>, we give another example of a complex graph pattern looking for food festivals or drinks festivals not held in Santiago, optionally returning their start date and name (where available).</p>
		...
		<figure id="fig-cgp">
			...
			<div><div style="display:inline;">\(Q := ((((Q_1 \cup Q_2) \rhd Q_3)\) <?php echo LeftJoin() ?> \(Q_4 )\) <?php echo LeftJoin() ?> \(Q_5),\qquad Q(G) =\) <table class="condensedTable" style="position:relative;top:.6em;display:inline-block;vertical-align:middle;"><thead><tr><th>?event</th><th>?start</th><th>?name</th></tr></thead><tbody><tr><td><code>EID16</code></td><td></td><td><code>Food Truck</code></td></tr></tbody></table></div></div>
			<figcaption>Complex graph pattern (\(Q\)) with mappings generated (\(Q(G)\)) over the graph of Figure&nbsp;<?php echo ref("fig:delg"); ?> (\(G\)) <a class="git" title="Consult the code for this example on Github" href="https://github.com/Knowledge-Graphs-Book/examples/blob/main/Chapter_2_Data_Graphs/2_2_2_Complex_graph_patterns/figure_2_7.sparql"></a></figcaption>
		</figure>
		...
		<div class="formal">
			...
			<dl class="definition" id="def-evalcgp">
				...
				<dd>Given a complex graph pattern \(Q\), if \(Q\) is a basic graph pattern, then \(Q(G)\) is defined per Definition&nbsp;<?php echo ref("def:evgp"); ?>. Otherwise, \(Q(G)\) is defined as follows:
				\begin{align*}
			...
			<p>Based on these operators, we can define some additional syntactic operators, such as the <em>left-join</em> (<?php echo LeftJoin(); ?>, aka <em>optional</em>):</p>
			<p>
			\begin{align*}
			 Q_1 <?php echo LeftJoin(false); ?> Q_2(G) = & \,(Q_1(G) \Join Q_2(G)) \cup (Q_1(G) \rhd Q_2(G))
			\end{align*}
			</p>
			<p>We call such operators <em>syntactic</em> as they do not add expressivity.</p>

			<div class="example">
				<p>Figure&nbsp;<?php echo ref("fig:cgp"); ?> illustrates a complex graph pattern and its evaluation.</p>
			</div>
		</div>
		...
		<p>Regular path queries can be evaluated under a number of different semantics. For example, \((\)<code>Arica</code>, <code>bus*</code>, <code>?city</code>\()\) evaluated against the graph of Figure&nbsp;<?php echo ref("fig:delg"); ?> may match the paths shown in Figure&nbsp;<?php echo ref("fig:path"); ?>. In fact, since a cycle is present, an infinite number of paths are potentially matched. For this reason, restricted semantics are often applied, returning only the shortest paths, or paths without repeated nodes or edges (as in the case of Cypher).<?php echo footnote("Mapping variables to paths requires special treatment&nbsp;". $references->cite("AnglesABHRV17") .". Cypher&nbsp;". $references->cite("FrancisGGLLMPRS18") ." returns a string that encodes a path, upon which certain functions such as <code>length(·)</code> can be applied. G-CORE&nbsp;". $references->cite("AnglesABBFGLPPS18") .", on the other hand, allows for returning paths, and supports additional operators on them, including projecting them as graphs, applying cost functions, and more besides."); ?> Rather than returning paths, another option is to instead return the (finite) set of pairs of nodes connected by a matching path (as in the case of SPARQL&nbsp;1.1).</p>
		...
		<figure id="fig-path">
			...
			<figcaption>Example paths matching \((\)<code>Arica</code>, <code>bus*</code>, <code>?city</code>\()\) over the graph of Figure&nbsp;<?php echo ref("fig:delg"); ?></figcaption>
		</figure>

		<p>Regular path queries can then be used in basic graph patterns to express <em>navigational graph patterns</em>&nbsp;<?php echo $references->cite("AnglesABHRV17"); ?>, as shown in Figure&nbsp;<?php echo ref("fig:ngp"); ?>, which illustrates a query searching for food festivals in cities reachable (recursively) from Arica by bus or flight. Furthermore, when regular path queries and graph patterns are combined with operators such as projection, selection, union, difference, and optional, the result is known as <em>complex navigational graph patterns</em>&nbsp;<?php echo $references->cite("AnglesABHRV17"); ?>.</p>
		...
		<div class="formal">
			...
			<p>We now define the evaluation of a path expression on a directed-edge labelled graph under the SPARQL 1.1-style semantics whereby the endpoints (pairs of start and end nodes) of the path are returned&nbsp;<?php echo $references->cite("sparql11"); ?>.</p>
			...
			<p>The inclusion of the reflexive pairs \((u,u)\) in the definition of \(r^*[G]\) captures zero-length paths. For example, in the query \((\)<code>Arica</code>, <code>bus*</code>, <code>?city</code>\()\), the reflexive pair \((\)<code>Arica</code>, <code>Arica</code>\()\) ensures that the variable <code>?city</code> will also match <code>Arica</code> via the zero-length path.</p>
			...
			<p>Query languages may support additional operators, some of which are syntactic (e.g., \(r^+\) is sometimes used for one-or-more, but can be rewritten as \(r \cdot r^*\)), while others may add expressivity such as the case of SPARQL&nbsp;<?php echo $references->cite("sparql11"); ?>, which allows a limited form of negation in expressions (e.g., \(!r\), with \(r\) being a constant or the inverse of a constant, matching any path not labelled \(r\)).</p>
			...
			<p>The definition of the evaluation of a navigational graph pattern then follows from the previous definition of a join and the definition of the evaluation of a regular path query (for a directed edge-labelled graph or a property graph, respectively). Likewise, <em>complex navigational graph patterns</em> – and their evaluation – are defined by extending this definition in the natural way with the same operators from Definition&nbsp;<?php echo ref("def:cgp"); ?> following the same semantics seen in Definition&nbsp;<?php echo ref("def:evalcgp"); ?>.</p>
		</div>

		<h4 id="app-qother" class="subsection">Other features</h4>
		<p>Thus far, we have discussed features that form the practical and theoretical foundation of any query language for graphs&nbsp;<?php echo $references->cite("AnglesABHRV17"); ?>. However, specific query languages for graphs may support other features, such as aggregation (<code>GROUP BY</code>, <code>COUNT</code>, etc.), more complex filters and datatype operators (e.g., range queries on years extracted from a date), federation for querying remotely hosted graphs over the Web, languages for updating graphs, support for entailment, etc. For more information, we refer to the documentation of the respective query languages (e.g.,&nbsp;<?php echo $references->cite("sparql11,AnglesABBFGLPPS18"); ?>) and to the survey by&nbsp;<?php echo $references->citet("AnglesABHRV17"); ?>.</p>
