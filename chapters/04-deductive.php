	<section id="chap-deductive" class="chapter">
		...
		<p>These assumptions (or lack thereof) define which interpretations are valid, and which interpretations <em>satisfy</em> which data graphs. We call an interpretation that satisfies a data graph a <em>model</em> of that data graph. The UNA forbids interpretations that map two data terms to the same domain term. The NUNA allows such interpretations. Under the CWA, an interpretation that contains an edge <? echo giedge("x","p","y"); ?> in its domain graph can only satisfy a data graph from which we can entail <? echo gedge("x","p","y"); ?>. Under the OWA, an interpretation containing the edge <? echo giedge("x","p","y"); ?> can satisfy a data graph not entailing <? echo gedge("x","p","y"); ?> so long it does not explicitly contradict that edge. OWL adopts the NUNA and OWA, which is the most general case: multiple nodes/edge-labels in the graph may refer to the same entity/relation-type (per the NUNA), and anything not entailed by the data graph is <em>not</em> assumed to be false as a consequence (per the OWA).</p>
		...
		<div class="formal">
			...
			<p>We do not restrict the language used to define semantic conditions, but, for example, we can define the <span class="sc">Has Value</span> semantic condition of Table&nbsp;<? echo ref("tab:ontClass"); ?> in FOL as:</p>
		...
		<figure id="fig-qrew">
			...
			<figcaption>Query rewriting example for the query \(Q\) of Figure&nbsp;<? echo ref("fig:bgpFS"); ?> <a class="git" title="Consult the code for this example on Github" href="https://github.com/Knowledge-Graphs-Book/examples/blob/main/Chapter_4_Deductive_Knowledge/4_2_1_Rules/"></a></figcaption>
		</figure>
		...
		<p class="mathblock"><img class="inside" src="images/fig-inline-rule.svg" alt="dom flight rule premise" style="margin-right:2em;vertical-align:middle;position:relative;"/> \(\Rightarrow\) <? echo sedge("?x","domestic&nbsp;flight",NULL,"?y","gvar"); ?> <a class="git" title="Consult the code for this example on Github" href="https://github.com/Knowledge-Graphs-Book/examples/blob/main/Chapter_4_Deductive_Knowledge/4_2_1_Rules/flight-rule.rif"></a></p>

		<p>Various languages allow for expressing rules over graphs – independently or alongside of an ontology language – including: Notation3 (N3)&nbsp;<? echo $references->cite("n3"); ?>, Rule Interchange Format (RIF)&nbsp;<? echo $references->cite("rif"); ?>, Semantic Web Rule Language (SWRL)&nbsp;<? echo $references->cite("swrl"); ?>, and SPARQL Inferencing Notation (SPIN)&nbsp;<? echo $references->cite("spin"); ?>, amongst others.</p>

		<div class="formal">
			<p>Given a graph pattern \(Q\) – be it a directed edge-labelled graph pattern per Definition&nbsp;<? echo ref("def:delgp"); ?> or a property graph pattern per Definition&nbsp;<? echo ref("def:pgp"); ?> – recall that \(\var(Q)\) denotes the variables appearing in \(Q\). We now define rules for graphs.</p>
		...
		<div class="formal">
			...
			<p>An interpretation \(I\) <em>satisfies</em> a knowledge-base \(\mathsf{K}\) if and only if, for all of the syntactic axioms in <span style="white-space:nowrap;">\(\mathsf{K}\),</span> the corresponding semantic conditions in Table&nbsp;<? echo ref("tab:dlsem"); ?> hold for \(I\). In this case, we call \(I\) a <em>model</em> of \(\mathsf{K}\).</p>
