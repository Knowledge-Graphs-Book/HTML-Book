...
<!--this is version from commit b715c7e6de1db08ef5bbe36ab043c027802ec357 -->
...
define('BIBTEXBROWSER','vb715c7e6de1db08ef5bbe36ab043c027802ec357');
...
@define('USE_FIRST_THEN_LAST',true); // put first names before last names?
...
//     echo $this->getKey();
  ...
  */
  ...
  function split_editors() {
    $array = preg_split('/ and( |$)/ims', @$this->raw_fields[EDITOR]);
    $res = array();
    // we merge the remaining ones
    for ($i=0; $i < count($array)-1; $i++) {
      if (strpos( latex2html($array[$i],false), '{') !== FALSE && strpos(latex2html($array[$i+1],false),'}') !== FALSE) {
  ...
  */
  ...
  /** Returns the editors as an array of strings (one string per editor).
   */
  function getFormattedEditorsArray() {
    $array_editors = array();


    // first we use formatAuthor
    foreach ($this->getRawEditors() as $editor) {
      $array_editors[]=$this->formatAuthor($editor);
    }

    return $array_editors;
  }
  ...
  */
  ...
  function getFormattedEditorsString() {
    if(count($this->getFormattedEditorsArray()) > 1) {
      $_ed = " (Eds.)";
    }
    else {
      $_ed = " (Ed.)";
    }
    return $this->implodeAuthors($this->getFormattedEditorsArray()) . $_ed;
  }
  ...
  */
  function getCompactedAuthors(){
    ...
    //return $this->getLastName($authors[0]) . $etal;
    return $this->formatAuthor($authors[0]) . $etal;
  }
  /**
  * Returns a compacted string form of editor names by throwing away
  * all editor names except for the first one and appending ", et al."
  */
  function getCompactededitors(){
    $editors = $this->getRawEditors();
    $etal = count($editors) > 1 ? ', et al.' : '';
    return $this->formatAuthor($editors[0]) . $etal;
  }
  ...
  /** Returns the editors of this entry as an array */
  function getEditors() {
    $editors = array();
  ...
  */
  ...
  function getPages() {
    preg_match('/([0-9]+\:?[0-9]+?).*?([0-9]+\:?[0-9]+?)/',$this->getField('pages'),$matches);
    array_shift($matches);
    return $matches;
  }
...
if (!function_exists('poweredby')) {
  ...
  function poweredby() {
    ...
    $poweredby .= 'Powered by <a href="http://www.monperrus.net/martin/bibtexbrowser/">bibtexbrowser</a><!--vb715c7e6de1db08ef5bbe36ab043c027802ec357-->';
    $poweredby .= '</div>'."\n";
    return $poweredby;
  }
}
...
<meta name="generator" content="bibtexbrowser vb715c7e6de1db08ef5bbe36ab043c027802ec357" />
<?php
...
<rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom">
   <channel>
      ...
      <generator>bibtexbrowser vb715c7e6de1db08ef5bbe36ab043c027802ec357</generator>
...
class Dispatcher {
  ...
  function diagnosis() {
    ...
    echo "bibtexbrowser version: b715c7e6de1db08ef5bbe36ab043c027802ec357\n";
